...
## How do I use the Plotly plugin or other visualization plugins in deployment?

The Plotly plugin is included by default in Superset 3.0+ and doesn't require any additional installation. For information on how to install custom visualization plugins in production environments (Docker, Kubernetes), see [Installing Visualization Plugins](/docs/installation/installing-plugins).
